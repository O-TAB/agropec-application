...
const DetailsPopup: React.FC<DetailsPopupProps> = ({ itemData, onClose, imageMap }) => {
  ...
  return (
    ...
    >
      ...
      >
        <img src={imageSrc} alt={itemData.title} className="w-full h-64 object-cover" />

        <div className="p-6 overflow-y-auto">
          <h2 className="text-3xl font-bold text-gray-800 mb-4">{itemData.title}</h2>
          ...
          <div className="flex items-center text-gray-700 bg-gray-100 p-3 rounded-lg">
            ...
            <span className="ml-2">{itemData.operatingHours}</span>
          </div>
        </div>
